...
import com.byteflow.learnffmpeg.recoder.SurfaceMediaRecorder;
...
public class GLMediaPlayerActivity extends AppCompatActivity implements GLSurfaceView.Renderer, FFMediaPlayer.EventCallback, MyGLSurfaceView.OnGestureCallback{
    ...
    private MediaFBORender mMediaFBORender;
    private SurfaceMediaRecorder mMediaRecorder;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMediaFBORender = new MediaFBORender(this);
    }
    ...
    public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
        ...
        mMediaFBORender.onSurfaceCreated();
    }
    ...
    public void onSurfaceChanged(GL10 gl10, int w, int h) {
        ...
        mMediaFBORender.onSurfaceChanged(w, h);
        if(mVideoHeight * mVideoWidth != 0) {
            float viewRatio = w * 1.0f / h;
            float frameRatio = mVideoWidth * 1.0f / mVideoHeight;
            if(viewRatio == frameRatio) {
                mMediaRecorder = new SurfaceMediaRecorder(mMediaFBORender);
    ...
    public void onDrawFrame(GL10 gl10) {
        mMediaFBORender.onPreDrawFrame();
        FFMediaPlayer.native_OnDrawFrame(VIDEO_GL_RENDER);
        mMediaFBORender.onDrawFrame();
    }
    ...
    private void onDecoderReady() {
        mVideoWidth = (int) mMediaPlayer.getMediaParams(MEDIA_PARAM_VIDEO_WIDTH);
        mVideoHeight = (int) mMediaPlayer.getMediaParams(MEDIA_PARAM_VIDEO_HEIGHT);
        if(mVideoHeight * mVideoWidth != 0)
            mGLSurfaceView.setAspectRatio(mVideoWidth, mVideoHeight);
